...
export default function BandCreatePage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      console.log('Creating band with data:', { name: bandName, description, user_id: user.id });
      
      // Use the band service to create the band
      const { data: band, error: bandError } = await bandService.createBand(bandName, description);
      
      if (bandError || !band) {
        const errorMsg = bandError?.message || 'Failed to create band';
        console.error('Error creating band:', errorMsg);
        setError(errorMsg);
        return;
      }

      console.log('Successfully created band, redirecting to band page...');
      navigate(`/bands/${band.id}`);
    } catch (err) {
      console.error('Unexpected error in band creation:', err);
          ...
          <form onSubmit={handleSubmit} className="p-8 space-y-6">
            ...
            <div>
              ...
              <input
                ...
                required
                aria-required="true"
                aria-describedby="band-name-help"
              />
              <p id="band-name-help" className="text-sm text-gray-500 mt-2">
                Choose a memorable name that represents your band's style
              </p>
            </div>
            ...
            <div>
              ...
              <textarea
                ...
                aria-describedby="band-description-help"
              />
              <p id="band-description-help" className="text-sm text-gray-500 mt-2">
                Describe your musical style, influences, and what events you're best suited for
              </p>
            </div>
